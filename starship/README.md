...
NPC ships follow a pre-computed **TraversalPlan** (4 phases: accelerate → coast → flip → brake) evaluated each micro tick via pure arithmetic, with no integration drift. Player ships receive direct thrust commands from the client.
...
| Gate / Wormhole | Instant at location (full stop required) | n/a |
...
Recipe quantities are in **units** (not stacks). `StackSize` is a cargo slot constraint only: `slots = ceil(units / stackSize)`. Starved factories don't tick. No spurious progress.
...
- All `JsonSerializerOptions` must set `IncludeFields = true` because `System.Numerics.Vector2` uses fields, not properties; the serialiser skips them otherwise.
...
- [ ] Warp/Jump transition ramps: enter/exit behaviour (sublight ramp vs fixed module duration vs hybrid); currently engages/exits instantly
